                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4>
                                        Edit - Company Data
                                        <a href="company.php" class="btn btn-danger float-right">BACK</a>
                                    </h4>
                                </div>
                                <div class="card-body bg-white">
                                    <form action="code.php" method="POST">
                                        <?php
                                        if (isset($_GET['co_id'])) //this is in url thats why get
                                        {
                                            $comp_id = $_GET['co_id'];
                                            $query = "SELECT * FROM tcomp_login WHERE co_id ='$comp_id' ";
                                            $query_run  = mysqli_query($conn, $query);

                                            foreach ($query_run as $item) :   //using for each loop to fetch data 
                                        ?>
                                                <input type="hidden" name="comp_id" value="<?= $item['co_id']; ?>">
                                                ...
                                                <div class="modal-body bg-white">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" name="co_name" value="<?= $item['co_name']; ?>" class="form-control" required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="">Company Description</label>
                                                        <textarea name="cdesc" class="form-control" rows="3" required><?= $item['cdesc']; ?></textarea>
                                                    </div>
                                                </div>
                                                <div class="modal-footer">
                                                    <a href="company.php" class="btn btn-secondary">Back</a>
                                                    <button type="submit" name="comp_update" class="btn btn-primary">Update</button>
                                                </div>
                                        <?php
                                            endforeach;
                                        } else {
